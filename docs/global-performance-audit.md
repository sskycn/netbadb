...
| Known execution architecture | One synchronous database owner/worker serializes requests; complete QueryResult is materialized before transport row admission | No new worker, same-StorageId concurrency, runtime dependency or claim that transport limits bound executor memory. |
...
| Public compatibility | No dependency, unsafe block, public API, persistent-format, protocol, schema, manifest or inspection-version change. QueryResult remains owned; row policies remain post-execution transport limits. |
