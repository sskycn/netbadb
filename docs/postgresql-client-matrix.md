...
| Deferred program + terminal shadow-column swap | Simple and Extended Query, bounded | Round 58 managed Single Heap: after frozen E, an effective DROP INDEX logically evacuates an old single-column non-unique binding, terminal DROP/RENAME creates final F, and same-name CREATE gets a fresh C4 index in one S2; unindexed Round 56 flow remains valid; type conversion/later DML closed |
