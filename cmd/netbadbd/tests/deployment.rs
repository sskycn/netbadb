...
fn test_directory(transport: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "netbadbd-v5-{transport}-{}-{sequence}",
        std::process::id()
    ))
}

fn manifest_fixture(transport: &str) -> (PathBuf, PathBuf) {
...
fn assert_daemon_uses_driven_manifest(postgres: bool) {
    ...
    let (directory, manifest) = manifest_fixture(transport);
...
#[test]
fn native_daemon_uses_v5_driven_without_an_adaptive_flag() {
    assert_daemon_uses_driven_manifest(false);
}

#[test]
fn postgres_daemon_uses_v5_driven_without_an_adaptive_flag() {
